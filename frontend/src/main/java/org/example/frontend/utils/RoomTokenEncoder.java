...
public class RoomTokenEncoder {
  ...
  public static String encode(String GUID, String cipher, String cipherMode, String paddingMode, String IV) {
    String token = GUID + JOIN_DELIMITER + cipher + JOIN_DELIMITER + cipherMode + JOIN_DELIMITER + paddingMode + JOIN_DELIMITER + IV;
  ...
  public static DecodedRoomToken decode(String encodedToken) {
    ...
    if (parts.length != 5) throw new IllegalArgumentException("Invalid token format: " + decoded);
